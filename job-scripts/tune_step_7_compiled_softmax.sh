#!/usr/bin/env bash
#SBATCH --job-name=tttj-step7-softmax
#SBATCH --output=job-scripts/outputs/tune_step_7_compiled_softmax-%j.out
...
uv run --frozen python -u -m profiling.tune_step_7_compiled_softmax "$@"
