#!/usr/bin/env bash
#SBATCH --job-name=tttj-step7
#SBATCH --output=job-scripts/outputs/run_step_7_h200-%j.out
...
if [[ "${GPU_NAME}" == *H200* && " $* " == *" --microbench "* ]]; then
    exec "${SLURM_SUBMIT_DIR}/job-scripts/run_step_7_h200_bundle.sh"
fi
